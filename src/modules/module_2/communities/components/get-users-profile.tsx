...
export default function ModalMembersCommunity({ currentUsers, isOpen, setIsOpen, communityName }:{ currentUsers:User[], isOpen:boolean, setIsOpen:Dispatch<SetStateAction<boolean>>, communityName:string }){
    ...
    return(
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-xl overflow-visible border border-gray-100 font-sans">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {currentUsers.length > 0 ? (
                            currentUsers.map((profile: User) => (
                                <article
                                    key={profile.id}
                                    className="relative flex items-start gap-3 rounded-xl border border-[#777d85] bg-white p-3"
                                >
                                    <Image
